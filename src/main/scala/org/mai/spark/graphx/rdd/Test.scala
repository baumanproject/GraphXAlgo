...
import org.mai.spark.graphx.rdd.Main.graph
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    logger.warn("Extracting data stage")
    extractData("aaa")

    logger.info("Data has been already extracted")

    val  graph = generateGraph(spark)

    val algo = new BreadthFirstSearch {}

    val result = algo.searchPathByDepth(graph, 1L,4)

    //result.vertices.foreach(println)
    //val a = result.vertices
    val a = result.vertices.filter(vertex=> vertex._2._2.asInstanceOf[(Double, List[Int])]._1 != Double.PositiveInfinity).count()
      //.map(t=>(t._1,(t._2._2.asInstanceOf[Tuple2[Double, List[Int]]]))
    println(s"For node 1L bws with depth 2 = ${a-1}")
    //a.foreach(println)
...
//    testConnectedComponents(spark)
  ...
  def extractData(path:String):Unit={
    val spark = createSparkSession(appName)
...
//    val sssp = algo.pairMinDistance(graph, 1L, 2L)
  ...
  def generateGraph(spark: SparkSession): Graph[String, Double] = {
    val vertices = spark.sparkContext.parallelize(List((1l, "A"), (2L, "B"), (3L, "C"), (4L, "D"), (5L,"E"),(6L,"F"), (7L,"Z"),(8L,"J")))
