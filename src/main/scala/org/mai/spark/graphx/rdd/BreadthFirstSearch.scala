...
//        .isEmpty()
  ...
  def searchPathByDepth[ED, VD](graph: Graph[ED, VD], sourceId: VertexId, depth: Integer) = {
    var g2 = graph.mapVertices( (vid, _) => if (vid == sourceId) (0.0, List(vid)) else (Double.PositiveInfinity, List.empty[VertexId]))
    ...
    while( !stop ) {
      ...
      val newDistances = g2.aggregateMessages[(Double, List[VertexId])](
        ctx => {
          if(ctx.srcAttr._1 < Double.PositiveInfinity && ctx.dstAttr._1 == Double.PositiveInfinity && ctx.srcAttr._1<depth) {
            ctx.sendToDst((ctx.srcAttr._1 + 1, ctx.srcAttr._2 :+ ctx.dstId))
          }
        },
        (a,b) => if(a._1 < b._1) a else b
      )
